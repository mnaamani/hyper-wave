...
- `app/` — the desktop MVP. See `app/README.md` for architecture and run commands. Key files: `app/workers/lib/wave.js` (runtime-agnostic P2P engine — where token race/gossip/Autobase/WDK get added), `app/workers/hyperwave.js` (Bare worker ↔ IPC bridge), `app/renderer/app.js` (ring UI, proof-window webcam, gallery). `app/electron/main.js` is the template plus one line: a `setPermissionRequestHandler` allowing `media` for the webcam.
