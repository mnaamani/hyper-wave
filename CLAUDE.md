...
- `docs/` — `architecture.md` (process/layer structure + module map + where logic lives) and `protocol.md` (on-wire protocol & state machine, spec-level: messages, crypto, lifecycle, gallery). Keep these in sync when the protocol or structure changes.
