...
// seed + N peers, drives a full wave, and asserts on the outcome via the harness's
...
// Launch a seed + N peers, all auto-joining and auto-selfie-ing; p1 kicks off once it sees
// everyone (the N-1 other peers + the seed = N others). Launches are staggered — the other
// half of reliable DHT discovery (see harness.start's warm-up). Returns { seed, peers }.
async function launchWave(c, seedEnv = {}) {
  const seed = c.launch('seed', { HYPERWAVE_ROLE: 'validator', ...seedEnv })
  const peers = []
  for (let i = 1; i <= N; i++) {
    peers.push(
      c.launch('p' + i, {
...
test(`a ${N}-peer wave converges the gallery on every node`, { timeout: 150000 }, async (t) => {
  ...
  const { seed, peers } = await launchWave(c)

  // every participant — and the archivist seed — ends up with all N selfies
  for (const p of peers) t.ok(await p.waitForGallery(N, 90000), `${p.name} converged to ${N}`)
  t.ok(await seed.waitForGallery(N, 90000), `the seed archived all ${N} selfies`)
...
test(
  ...
  async (t) => {
    ...
    // once the ball is moving, kill two mid-ring peers
    ...
    // and every surviving participant still posts, so the gallery reaches at least N-2
    t.ok(
      await seed.waitForGallery(N - 2, 90000),
      `gallery reached ≥ ${N - 2} (survivors all posted)`
    )
  }
)
...
test(
  ...
  async (t) => {
    ...
    const { seed } = await launchWave(c, { HYPERWAVE_RAFFLE_TRX: '3' })

    // after the wave the seed folds every reveal into a deterministic draw over all N tickets
    const draw = await seed.waitForEvent('raffle-draw', 90000)
