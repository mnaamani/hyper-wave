...
> demos use a local DHT bootstrap (`bare packages/hyperwave-engine/bin/dht-local.js` +
...
- **`workers/hyperwave.js`** (Bare, CJS) — a ~40-line host: wraps `Bare.IPC` in a
  `FramedStream` and calls `hyperwave-engine`'s `createEngine()`. All P2P/protocol/wallet logic
...
`packages/hyperwave-engine` and run from the repo root:
